...
class TestBridgeCountFunction(unittest.TestCase):
    ...
    def test_many_bridge_count(self):
        env = Tetris(8, 16)
        for _ in range(len(tetris_shapes)):
            env.shape = tetris_shapes[0]
            env.shape_x = 4
            env.hard_drop()
...
class TestCountFullLinesFunction(unittest.TestCase):
    def test_count_full_lines(self):
        env = Tetris(8, 16)
        env._place_shape(env.board, tetris_shapes[5], (0, 1))
        env._place_shape(env.board, tetris_shapes[5], (4, 1))
